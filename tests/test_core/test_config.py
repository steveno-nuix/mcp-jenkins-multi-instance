...
class TestJenkinsInstanceConfig:
    def test_defaults(self):
        config = JenkinsInstanceConfig(url='http://j.example.com', username='u', password='p')
        assert config.timeout == 5
        assert config.verify_ssl is True

    def test_custom_values(self):
        config = JenkinsInstanceConfig(
            url='http://j.example.com', username='u', password='p', timeout=10, verify_ssl=False
        )
...
"""
        ...
        assert config.instances['discover'].url == 'https://jenkins-discover.example.com'
