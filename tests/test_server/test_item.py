...
async def test_get_item(mock_jenkins, mocker):
    mock_jenkins.get_item.return_value = Job(
        fullname="job1", color="blue", name="job1", url="1", class_="Job"
    )
...
async def test_get_item_config(mock_jenkins, mocker):
    ...
    assert (
        await item.get_item_config(mocker.Mock(), fullname="job1")
        == "<xml>config</xml>"
    )
...
async def test_set_item_config(mock_jenkins, mocker):
    ...
    await item.set_item_config(
        mocker.Mock(), fullname="job1", config_xml="<xml>config</xml>"
    )
