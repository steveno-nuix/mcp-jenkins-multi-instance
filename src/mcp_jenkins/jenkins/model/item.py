...
ItemType = Union[
    "Folder", "MultiBranchProject", "FreeStyleProject", "Job", "UnknownItem"
]
...
def serialize_item(item: dict) -> ItemType:
    ...
    target_cls = next(
        (cls for name, cls in cls_map.items() if _class.endswith(name)), UnknownItem
    )
    ...
    if "jobs" in item and isinstance(item["jobs"], list):
        item = {
            **item,
            "jobs": [
                serialize_item(job) if isinstance(job, dict) else job
