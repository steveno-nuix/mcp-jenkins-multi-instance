...
class AuthMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        jenkins_username = (
            jenkins_username_bytes.decode("latin-1") if jenkins_username_bytes else None
        )
        jenkins_password = (
            jenkins_password_bytes.decode("latin-1") if jenkins_password_bytes else None
        )
        jenkins_instance = (
            jenkins_instance_bytes.decode("latin-1") if jenkins_instance_bytes else None
        )
